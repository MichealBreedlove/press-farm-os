    ...
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1">
        <button onClick={() => setFilterCat("all")}
          className={`px-3 py-1.5 rounded-full text-xs font-medium min-h-0 whitespace-nowrap ${filterCat === "all" ? "bg-farm-green text-white" : "bg-farm-cream/60 text-farm-muted"}`}
        >All</button>
        {categories.map((c) => (
          <button key={c} onClick={() => setFilterCat(c)}
            className={`px-3 py-1.5 rounded-full text-xs font-medium min-h-0 whitespace-nowrap ${filterCat === c ? "bg-farm-green text-white" : "bg-farm-cream/60 text-farm-muted"}`}
          >{c}</button>
        ))}
      </div>

      {/* Mobile scroll hint — only shows below 640px since the timeline needs horizontal scroll there */}
      <p className="sm:hidden text-[11px] text-farm-muted/80 italic px-1">
      ...
      <div className="card overflow-hidden overflow-x-auto">
        {/* Month header */}
        <div className="flex border-b border-farm-dark/5 bg-farm-cream/40 sticky top-0 z-10 min-w-[600px]">
          <div className="w-24 sm:w-40 flex-shrink-0 px-3 py-2 text-xs font-semibold text-farm-muted">Crop</div>
          <div className="flex-1 flex">
            {MONTHS.map((m) => (
              <div key={m} className="flex-1 text-center py-2 text-[10px] font-medium text-farm-muted border-l border-farm-dark/5">
                {m}
              </div>
            ))}
          </div>
          <div className="w-20 text-center py-2 text-[10px] font-medium text-farm-muted border-l border-farm-dark/5">Info</div>
        </div>

        {/* Today marker column index */}
        {grouped.length === 0 ? (
          <div className="text-center py-12">
            <Sprout className="w-8 h-8 text-farm-muted/60 mx-auto mb-2" />
            <p className="text-sm text-farm-muted">No plantings yet</p>
            <button onClick={() => setShowAdd(true)} className="text-sm text-farm-green font-medium mt-2 min-h-0">
              + Add your first planting
            </button>
          </div>
        ) : (
          <div className="divide-y divide-farm-dark/5 max-h-[60vh] overflow-y-auto">
                  ...
                  {isExpanded && cropPlantings.map((p) => (
                    ...
                    >
                      <div className="w-24 sm:w-40 flex-shrink-0 pl-8 pr-3 py-2">
                        <p className="text-[11px] text-farm-muted/90 truncate">{p.variety || p.crop_name}</p>
                        <p className="text-[9px] text-farm-muted">{p.location ?? "No location"}</p>
                      </div>
        ...
        {/* Legend */}
        <div className="px-3 py-2 border-t border-farm-dark/5 flex flex-wrap gap-3">
          {Object.entries(CAT_COLORS).filter(([k]) => categories.includes(k)).map(([k, c]) => (
            <div key={k} className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: c }} />
              <span className="text-[10px] text-farm-muted capitalize">{k}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="px-4 py-3 border-b border-farm-dark/5 flex items-center justify-between">
          <div>
            ...
            {step === "details" && selectedItem?.image_url && (
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-farm-muted">{form.category}</span>
              </div>
            )}
          </div>
          <button onClick={onClose} className="p-2 text-farm-muted hover:text-farm-muted/90 min-h-0 min-w-0">
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <div className="px-4 pb-4 space-y-1 max-h-[50vh] overflow-y-auto">
              {filteredItems.map((item) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-farm-cream/40 transition-colors text-left min-h-0"
                >
                  {item.image_url ? (
                    <img src={item.image_url} alt="" className="w-8 h-8 rounded-lg object-cover flex-shrink-0" />
                  ) : (
                    <div className="w-8 h-8 rounded-lg bg-farm-cream/60 flex items-center justify-center flex-shrink-0">
                      <Sprout className="w-4 h-4 text-farm-muted/60" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-farm-dark truncate">{item.name}</p>
                    <p className="text-xs text-farm-muted">{item.category} · {item.unit_type.toUpperCase()}{item.default_price ? ` · $${item.default_price}` : ""}</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        {step === "details" && (
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-semibold text-farm-muted uppercase tracking-wider">Planting Timeline</h4>
              ...
              {form.sow_date && form.harvest_end && (
                <div className="bg-farm-cream/40 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-1 text-[9px] text-farm-muted mb-1">
                  ...
                  <div className="flex justify-between text-[8px] text-farm-muted mt-0.5">
                    <span>Growing</span>
            ...
            {/* Growing info */}
            <div className="space-y-3">
              <h4 className="text-xs font-semibold text-farm-muted uppercase tracking-wider">Growing Info</h4>
              <div className="grid grid-cols-2 gap-3">
                <div>
            ...
            {/* Quantity & Revenue */}
            <div className="space-y-3">
              <h4 className="text-xs font-semibold text-farm-muted uppercase tracking-wider">Amount & Revenue</h4>
              <div className="grid grid-cols-3 gap-3">
                <div>
        ...
        {/* Footer */}
        <div className="px-4 py-3 border-t border-farm-dark/5 flex gap-2">
          {step === "details" && (
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[80vh] flex flex-col">
        <div className="px-4 py-3 border-b border-farm-dark/5 flex items-center justify-between">
          <div>
            ...
            {planting.variety && <p className="text-xs text-farm-muted">{planting.variety}</p>}
          </div>
          <button onClick={onClose} className="p-2 text-farm-muted min-h-0 min-w-0"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            ...
            <div className="flex gap-1 ml-auto">
              {["planned", "planted", "growing", "harvesting", "terminated"].map((s) => (
                <button key={s} onClick={() => updateStatus(s)}
                  className={`text-[9px] px-2 py-0.5 rounded-full min-h-0 min-w-0 capitalize ${planting.status === s ? "bg-farm-green text-white" : "bg-farm-cream/60 text-farm-muted"}`}
                >{s}</button>
              ))}
            </div>
          </div>
          ...
          <div className="card p-3 space-y-2">
            <h4 className="text-xs font-semibold text-farm-muted">Timeline</h4>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div><span className="text-farm-muted">Sow:</span> <span className="font-medium">{fmtDate(planting.sow_date)}</span></div>
              <div><span className="text-farm-muted">Maturity:</span> <span className="font-medium">{planting.days_to_maturity ?? "—"}d</span></div>
              <div><span className="text-farm-muted">Harvest Start:</span> <span className="font-medium">{fmtDate(planting.harvest_start)}</span></div>
              <div><span className="text-farm-muted">Harvest End:</span> <span className="font-medium">{fmtDate(planting.harvest_end)}</span></div>
            </div>
          </div>

          {/* Details */}
          <div className="card p-3 space-y-2">
            <h4 className="text-xs font-semibold text-farm-muted">Details</h4>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div><span className="text-farm-muted">Category:</span> <span className="font-medium capitalize">{planting.category ?? "—"}</span></div>
              <div><span className="text-farm-muted">Location:</span> <span className="font-medium">{planting.location ?? "—"}</span></div>
              <div><span className="text-farm-muted">Growing:</span> <span className="font-medium capitalize">{planting.growing_location?.replace("_", " ") ?? "—"}</span></div>
              <div><span className="text-farm-muted">Stock:</span> <span className="font-medium capitalize">{planting.planting_stock ?? "—"}</span></div>
              <div><span className="text-farm-muted">Quantity:</span> <span className="font-medium">{planting.quantity ?? "—"} {planting.quantity_unit ?? ""}</span></div>
              <div><span className="text-farm-muted">Avg Price:</span> <span className="font-medium">{planting.avg_price ? `$${planting.avg_price}` : "—"}</span></div>
            </div>
          </div>
          ...
          {planting.notes && (
            <div className="card p-3">
              <h4 className="text-xs font-semibold text-farm-muted mb-1">Notes</h4>
              <p className="text-xs text-farm-dark/80">{planting.notes}</p>
            </div>
          )}
        </div>
