  ...
  return (
    <main className="pb-24">
      ...
      <div className="px-4 py-6 space-y-5">
        ...
        <div>
          <p className="text-xs font-medium text-farm-muted mb-2">Import Type</p>
              ...
              <button
                ...
                className={`min-h-[48px] rounded-xl border text-sm font-medium transition-colors ${
                  target === t
                    ...
                    : "bg-white text-farm-dark/80 border-farm-dark/10 hover:border-gray-400"
                }`}
              >
        ...
        {/* File picker */}
        <div>
          <p className="text-xs font-medium text-farm-muted mb-2">Upload File (.xlsx)</p>
          <label className="block w-full min-h-[56px] border-2 border-dashed border-farm-dark/10 rounded-xl flex items-center justify-center cursor-pointer hover:border-gray-400 transition-colors">
            <input
              ref={fileInputRef}
              type="file"
              accept=".xlsx,.xls"
        ...
        {file && !result && (
          <div className="flex gap-3">
            <button
              ...
              disabled={loading}
              className="flex-1 min-h-[44px] bg-farm-cream/60 text-farm-dark/80 rounded-xl text-sm font-medium disabled:opacity-50 hover:bg-gray-200 transition-colors"
            >
        ...
        {preview && (
          <div className="space-y-3">
            <div className="bg-farm-cream/60 border border-farm-green/15 rounded-xl p-4">
            ...
            {preview.preview && preview.preview.length > 0 && (
              <div className="bg-white rounded-xl border border-farm-dark/5 overflow-hidden">
                <p className="px-4 pt-3 pb-2 text-xs font-medium text-farm-muted">
                  ...
                  {preview.preview.map((row, i) => (
                    <div key={i} className="px-4 py-2 flex items-center justify-between gap-2 text-sm">
                      <div className="min-w-0">
                        <span className="font-medium text-farm-dark truncate block">{row.name}</span>
                        <span className="text-xs text-farm-muted">{row.category} · {row.unit}</span>
                      </div>
                      <span className="text-farm-dark/80 font-medium flex-shrink-0">${row.price.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="divide-y divide-farm-dark/5">
                  {preview.sample.map((s, i) => (
                    <div key={i} className="px-4 py-2 text-sm">
                      <p className="font-medium text-farm-dark">{s.key.replace("::", " — ")}</p>
        ...
        {result && (
          <div className="bg-farm-green-light border border-farm-green/20 rounded-xl p-4 space-y-2">
            ...
            {result.skipped > 0 && (
              <p className="text-xs text-farm-muted">{result.skipped} rows skipped (missing data)</p>
            )}
            <button
              onClick={reset}
