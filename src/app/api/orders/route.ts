...
export async function POST(request: Request) {
  ...
  if (delivery_date < minOrderableDatePacific()) {
    return NextResponse.json(
      {
        error: `Ordering for ${delivery_date} has closed (orders after ${ORDER_CUTOFF_LABEL} go to the next harvest day). Please go back to the order page and resubmit for the next delivery date.`,
      },
      { status: 409 },
    );
  }
