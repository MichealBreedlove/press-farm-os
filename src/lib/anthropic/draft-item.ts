...
export const DRAFT_ITEM_SCHEMA = {
  type: "object",
...
export const DRAFT_ITEM_SYSTEM_PROMPT = `You are a horticulture expert helping a small organic farm in Yountville, California (Napa Valley, Mediterranean climate, USDA zone 9b). The farm grows produce, herbs, flowers, and microgreens for high-end restaurants in the area.
...
export interface DraftItemOutput {
  chef_notes: string;
...
/**
 * Single-item drafter. Calls Claude Haiku 4.5 with the structured
 * output schema and returns the parsed result. Throws on any failure
 * — caller decides how to handle. The single-item route surfaces the
 * error to the form; the bulk-fill route catches and records per-item
 * so one failure doesn't bring down the whole batch.
 */
export async function draftItemContent(
  client: Anthropic,
  input: DraftItemInput,
): Promise<DraftItemOutput> {
  ...
  const userMessage = [
    ...
    color && `Color: ${color}`,
    category && `Category: ${CATEGORY_LABELS[category as ItemCategory] ?? category}`,
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 1024,
    system: DRAFT_ITEM_SYSTEM_PROMPT,
    output_config: {
      format: {
        type: "json_schema",
        schema: DRAFT_ITEM_SCHEMA as any,
      },
    },
  ...
  return JSON.parse(textBlock.text) as DraftItemOutput;
}
