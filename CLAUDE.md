...
src/
  app/
    ...
    harvest/                     # Harvester portal — combined pick list + add extras, EN/ES toggle
    ...
    api/
      ...
      cron/                      # tasks-regenerate, receiver-notify, weekly-update-watchdog (Bearer CRON_SECRET, fail-closed)
...
supabase/migrations/             # 001 → 079 (86 files; see table — 044/047/062 used twice, 069 three times)
```

## Database Migrations

Base schema + microgreens (045), seed inventory (046), inbound-email/inbox (060: `inbound_messages`, `inbox_task_drafts`), order accountability (058: `order_audit`), and farm tasks (062: `farm_tasks`). **86 migration files, numbered 001 → 079** — numbers **044, 047, and 062 are each used by two files, and 069 by three** that all shipped (historical collisions; don't "fix" them). 064 confirmed applied — the live security advisors are clean; 066 + 067 + 068 applied via the Supabase MCP; 069–071 shipped with their features (production value, event orders, substitutions); 072 (variety selection), 073 (harvester role), 074 (lemon verbena), 075 (weekly update log), 076 + 077 (delivery backfill & top-up), 078 (Press availability restore), and 079 (microgreens fall-2026 reset) applied via the Supabase MCP. **Next migration number is 080.** Read latest first when scoping work.
...
| 073 | harvester_role | Adds `harvester` role (CHECK constraint + `private.is_harvester()` + read-only RLS) for the /harvest field-crew portal. NOTE: prod's role helpers (is_admin/is_receiver/user_restaurant_ids) live in an untracked `private` schema — is_harvester matches that pattern |
| 074 | lemon_verbena | Adds Lemon Verbena to the catalog (data-only, modeled on Lemon Balm) — applied live 2026-07-26 via the Supabase MCP |
...
| 079 | microgreens_fall_2026_reset | DATA FIX — Micheal's 2026-09-19 "remove all microgreens from the schedule, we are starting over": deletes all 12 `microgreen_demand` rows, terminates the 31 in-flight trays (10 batches sown 08-14, never harvested; kept as history), supersedes the 24 open `microgreens-auto` tasks, deactivates all 74 crops and reactivates only the fall greenhouse lineup (Borage, Fava Tips, Pea Tendrils, Pea/Calvin, Pea/Frilly, Mustard Frills, New Zealand Spinach [new row, estimated timings; dropped again 2026-09-20 — row left inactive], Nasturtium, Gem Marigold, Chervil) with catalog links + a 1-tray = 1-LG yield default. Demand is intentionally left EMPTY — Micheal enters fall targets in /admin/microgreens/demand. Deleted demand rows are listed in the file's rollback notes. Applied 2026-09-19 via the Supabase MCP |
...
- **Harvester**: `harvester` role for field crew — sees ONLY `/harvest` (combined pick list + add-extra), bilingual EN/ES toggle. No restaurant assignment needed.
...
**Migrations**
- Numbered sequentially in `supabase/migrations/NNN_description.sql`. Next is **080**. (044/047/062 each collide across two files and 069 across three — don't add to those numbers. Check `ls supabase/migrations/ | tail` before numbering — CLAUDE.md's "next" note can lag sessions that shipped in parallel.)
...
- **Chef Weekly Update** (`/api/reports/weekly-update`, `src/emails/weekly-update.tsx`) — Monday-afternoon (cron `30 22 * * 1` = 3:30pm PDT; Vercel crons are UTC-fixed so the Pacific clock time shifts an hour at DST) "what's going on at the farm" email sent ONLY to a hand-picked recipient list (chef checkboxes + free-form addresses — chefs are NOT auto-included): general note, Available Now, planter beds, gaps/limited, incoming timeline. Managed on its own page `/admin/weekly-update`, where EVERY section of the email is editable pre-send: the page loads a live-built draft (or this week's saved edits), rows can be added/removed/changed, "Save Draft" persists to `weekly_update_draft` (JSON envelope `{weekOf, data}`, builder/sanitizer in `src/lib/weekly-update.ts`), and both Send Now and the Monday cron send the same-week draft verbatim (live build only as fallback). A manual send stamps `weekly_update_last_sent_week` so the cron skips that week. Other settings keys: `weekly_update_general_note` / `weekly_update_recipients`. **Every attempt — cron or manual, success, failure or skip — writes a `weekly_update_log` row**, and a watchdog (`/api/cron/weekly-update-watchdog`, `30 23 * * 1`, one hour after the send) emails `ADMIN_EMAIL` if that week's Monday has no `sent`/`partial` row, stamping `weekly_update_alert_sent_week` so it alerts at most once per week. This exists because the 2026-08-17 update silently failed: the last-sent stamp is only written on success, so "send failed" and "cron never fired" were indistinguishable and nothing surfaced either. The admin dashboard shows an amber reminder banner until the note has been saved since last Monday (or while the recipient list is empty). The internal financial weekly digest similarly supports a recipient list via `weekly_digest_recipients`.
