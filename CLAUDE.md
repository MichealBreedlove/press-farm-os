...
| 079 | normalize_unit_type_case | DATA FIX — lowercases `items.unit_type` + `unit_prices` keys (8 items stored `EA`, created 2026-06-13/14, which the case-sensitive item API validation made unsavable with "Invalid unit_type"). Code fix: `/api/items` + `/api/items/[itemId]` lowercase unit codes before validating. Idempotent. Applied 2026-08-30 via the Supabase MCP |
