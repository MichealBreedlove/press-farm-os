...
Base schema + microgreens (045), seed inventory (046), inbound-email/inbox (060: `inbound_messages`, `inbox_task_drafts`), order accountability (058: `order_audit`), and farm tasks (062: `farm_tasks`). **70 migration files, numbered 001 → 067** — numbers **044, 047, and 062 are each used by two different files** that both shipped (historical collisions; don't "fix" them). Last applied in prod: **067** (064 confirmed applied — the live security advisors are clean; 066 + 067 applied via the Supabase MCP). **Next migration number is 068.** Read latest first when scoping work.
