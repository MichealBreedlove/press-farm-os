...
--
-- Press Farm sells microgreens in LG / SM / EA / GB to-go containers,
-- not ounces. The yield model is "alternatives" — one tray of Pea
-- Shoots yields *either* 4 LG to-gos *or* 8 SM to-gos *or* 32 EA bags,
-- depending on what's been ordered.
--
...
COMMENT ON COLUMN microgreen_crops.yield_per_tray IS
  'Map of unit code to alternative yield per tray. Example: {"lg": 4, "sm": 8, "ea": 32} — one tray yields 4 LG OR 8 SM OR 32 EA, depending on packing decision.';
...
COMMENT ON COLUMN microgreen_demand.target_unit IS
  'Unit code (lg/sm/ea/gb). Must match a key in microgreen_crops.yield_per_tray for sow planning to compute trays_needed.';
