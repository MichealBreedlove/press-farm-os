-- Migration 067: order-submit idempotency + preserve submitted_at
--
-- Two fixes to the atomic submit path introduced in 066:
--
-- F-1 (double-submit): POST /api/orders selects "merge" mode whenever an order
-- already exists and the caller isn't in the explicit edit flow. A chef on a
-- flaky mobile connection whose response is LOST will retry, and the retry
-- merges (sums) the same quantities onto the lines that already committed —
-- silently doubling the order. We add an idempotency token: the client mints
-- one token per review-session submission and reuses it across retries; a
-- repeat with the same token is a no-op that returns the existing order.
--
-- F-2 (submitted_at clobbered): 066's ON CONFLICT set submitted_at = now() on
-- EVERY write, so each chef edit shifted the admin/chef "Submitted at" display
-- to the edit time and defeated the separate last_edited_at field. COALESCE
-- keeps the first submission time (still stamping it on draft -> submitted,
-- where it was NULL).
--
-- The route still does validation, price resolution, and merge planning in JS;
-- this function only applies the planned writes atomically. SECURITY INVOKER
-- so the chef's RLS keeps applying to every statement inside.

ALTER TABLE orders ADD COLUMN IF NOT EXISTS last_submission_token text;

-- The signature changes (new trailing param), so drop the 066 overload first
-- rather than leaving two functions resolvable by PostgREST.
DROP FUNCTION IF EXISTS public.submit_order_with_items(
  uuid, date, text, uuid, uuid, boolean, jsonb, jsonb
);
...
BEGIN
  ...
  -- FOR UPDATE serializes concurrent retries; if the existing order already
  -- carries this exact token, the original write committed and this is a
  -- retry. Return its id without re-applying any writes (which, in merge
  -- mode, would double the quantities).
...
-- Same grant hygiene as migrations 043/064/066: callable by signed-in users only.
REVOKE EXECUTE ON FUNCTION public.submit_order_with_items(uuid, date, text, uuid, uuid, boolean, jsonb, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.submit_order_with_items(uuid, date, text, uuid, uuid, boolean, jsonb, jsonb, text) TO authenticated, service_role;
